    ...
    for(int i = 0; i <= (int)strlen(parametros); i++){
        switch(estado){
            ...
            case 1: {
                ...
                else if((char)tolower(parametros[i]) == 's') estado = 17;
            ...
            case 14: {
                parametroActual += parametros[i];
                if(parametros[i] == '\"') {vNombre = false; nombreParticion = ""; estado = 15;}
                else if(parametros[i] == 9 || parametros[i] == 32) ;
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
                else {vNombre = false; nombreParticion = ""; nombreParticion += parametros[i]; estado = 16;}
                break;
            }
            case 15: {
                parametroActual += parametros[i];
            ...
            case 17: {
                parametroActual += parametros[i];
                if((char)tolower(parametros[i]) == 'i') estado = 18;
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
                else {estado = -1;}
                break;
            }
            case 18: {                
                parametroActual += parametros[i];
                if((char)tolower(parametros[i]) == 'z') estado = 19;
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
                else {estado = -1;}
                break;
            }
            case 19: {
                parametroActual += parametros[i];
                if((char)tolower(parametros[i]) == 'e') estado = 20;
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
                else {estado = -1;}
                break;
            }
            ...
            case 21: {
                parametroActual += parametros[i];
                if(isNumber(parametros[i])) {tamanoParticionStr += parametros[i]; estado = 22;}
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
                else {estado = -1;}
                break;
            }
            case 22: {
                ...
                if(isNumber(parametros[i])) tamanoParticionStr += parametros[i];
                    ...
                    if(stoi(tamanoParticionStr) <= 0) cout << "Error: El parametro -s del comando fdisk debe ser entero positivo \n";
            ...
            case 28: {
                ...
                if(isLetter(parametros[i])) tamanoUnitStr += parametros[i];
                    ...
                    else {cout << "Error: No se reconocio una unidad en el parametro " << parametroActual << " del comando fdisk \n";}
                    parametroActual = "";
                    estado = 0;
                }
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
            ...
            case 34: {
                ...
                if(isLetter(parametros[i])) tipoParticionStr += parametros[i];
                    ...
                    else {cout << "Error: No se reconocio un tipo de partición en el parametro " << parametroActual << " del comando fdisk \n";}
                    parametroActual = "";
                    estado = 0;
                }
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
            ...
            case 38: {
                parametroActual += parametros[i];
                if(isLetter(parametros[i])) {vFit = false; particionFitStr = parametros[i]; estado = 39;}
                else if(parametros[i] == 9 || parametros[i] == 32) ;
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
                else {estado = -1;}
                break;
            ...
            case 39: {
                parametroActual += parametros[i];
                if(isLetter(parametros[i])) particionFitStr += parametros[i];
                else if(parametros[i] == 0 || parametros[i] == 9 || parametros[i] == 32){
                    if(strcasecmp(particionFitStr.c_str(), "bf") == 0) {vFit = true; particionFit = 'B';}
                    else if(strcasecmp(particionFitStr.c_str(), "wf") == 0) {vFit = true; particionFit = 'W';}
                    else if(strcasecmp(particionFitStr.c_str(), "ff") == 0) {vFit = true; particionFit = 'F';}
                    else {cout << "Error: No se reconocio un fit en el parametro " << parametroActual << " del comando fdisk \n";}
                    parametroActual = "";
                    estado = 0;
                }
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
            ...
            case 47: {
                ...
                if(isLetter(parametros[i])) eliminarTipo += parametros[i];
                    ...
                    else {cout << "Error: No se reconocio un tipo de eliminación en " << parametroActual << " del comando fdisk \n";}
                    parametroActual = "";
                    estado = 0;
                }
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
            ...
            case 51: {
                ...
                else if (isNumber(parametros[i])){vAdd = false; cambiarTamanoStr = parametros[i]; estado = 50;}
                else if(parametros[i] == 9 || parametros[i] == 32) ;
                else if(parametros[i] == '#') {comentario = ""; comentario += parametros[i]; estado = -2;}
                else {estado = -1;}
                break;
            ...
            case -1: {
                if(parametros[i] == 0 || parametros[i] == 9 || parametros[i] == 32){
                    cout << "Error: " << parametroActual << " es invalido para el comando fdisk \n";
                    parametroActual = "";
                    estado = 0;
                }
    ...
    if(vPath && vNombre){
        if(vTamano){
            if(!vAdd && !vEliminado){
                ...
                if(vFit) fd.partitionFit = particionFit;
